...
/// Creates a string representing the given values.
///
/// Use the `§` operator to convert instances of any type to their preferred
/// representations as a single `String` instance. For each `instance` in
/// `instances`, the `§` operator creates the string representation of
/// `instance` in one of the following ways, depending on its protocol
/// conformance:
///
/// - If `instance` conforms to the `TextOutputStreamable` protocol, the
///   result is obtained by calling `instance.write(to: s)` on an empty
...
///     print(§p)
///     // Prints "(21, 30)"
//@inlinable public prefix func § (_ instances: (Any...)) -> String {
//    String(describing: instances)
//}
